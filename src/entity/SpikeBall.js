...
Crafty.c("SpikeBall", 
{
    init: function() 
    {
        this.requires("2D, Canvas, Delay, ICollisionDetection, Collision, IBase, IReset, IMortal");
        this.setScale(70, 70);
        this.curVel = { x: 0, y: 0 };
        ...
        this.hasImpacted = false; // Used for rolling, when initilized, it waits for the first collision to happen before applying x velocities
        this.affectCurVel = false;

        this.spriteObj = Crafty.e("SpriteSpikeBall");
    ...
    */
    ...
    callbackPostResolution: function(args) {
        ...
        {
            if (args.direction == "floor")
            {
                if (this.waitForImpact == true && this.hasImpacted == false) 
                {
                    this.curVel.x = this.resetVel.x;
